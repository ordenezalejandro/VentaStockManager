...
document.addEventListener("DOMContentLoaded",
    ...
    function(event) {
        ...
        function agregarEventosANuevoInline(nuevoInline) {
            const selectArticulo = nuevoInline.querySelector("select[id^='id_ventas-'][id$='-articulo']");
            const inputCantidad = nuevoInline.querySelector("input[id$='cantidad']");
            const inputPrecio = nuevoInline.querySelector("input[id$='precio']");
    
            if (selectArticulo) {
                selectArticulo.onchange = function() {
                    let select = selectArticulo;
                    let indice = get_indice(selectArticulo.id, "-articulo");
                    let articulo_venta = select_to_articulo_venta(null, text=select.options[select.selectedIndex].text);
                    let cantidad_node = document.querySelector(`#id_ventas-${indice}-cantidad`);
                    let precio_node = document.querySelector(`#id_ventas-${indice}-precio`);
                    let total_node = document.querySelector(`tr#ventas-${indice} td.field-precio_total`);
                    ...
                    update_precio_total();
                }// Aquí puedes agregar el código que se ejecutará cuando cambie el select de artículo
            }

    
            if (inputCantidad) {
                inputCantidad.onchange = function() {
                    let item = inputCantidad;
                    let indice = get_indice(item.id, "-cantidad");
                    let cantidad = item.value;
                    let price_node = get_price_node(indice);
                    let select_articulo_venta = document.querySelector(`#id_ventas-${indice}-articulo`);
                    ...
                    update_precio_total();
                // Aquí puedes agregar el código que se ejecutará cuando cambie la cantidad
                };
            }
    
            if (inputPrecio) {
                inputPrecio.onchange = function() {
                    let item = inputPrecio;
                    let indice = get_indice(item.id, "-precio");
                    let cantidad = get_cantidad_node(indice).value;
                    let precio = item.value;
                    let select_articulo_venta = document.querySelector(`#id_ventas-${indice}-articulo`);
                    ...
                    update_precio_total();
                }  // Aquí puedes agregar el código que se ejecutará cuando cambie el precio
            };
        ...
        // Observa cambios en el grupo de inlines
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    mutation.addedNodes.forEach(function(node) {
                        // if (node.classList && node.classList.contains('dynamic-ArticuloVentaInline')) {
                            console.log('Se ha agregado un nuevo inline de ArticuloVenta');
                            agregarEventosANuevoInline(node);// Aquí puedes añadir más lógica si necesitas hacer algo específico cuando se añade un nuevo inline
                        // }
                    });
                }
            });
        ...
        // Configura el observador para monitorear la adición de elementos al DOM
